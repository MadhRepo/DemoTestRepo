...
public class NseStockInfoDataDrivenTest extends BaseStockTest {
    ...
    public void testStockInformationDisplay(StockInfo expected, int rowIndex, String tcId) throws InterruptedException {
        ...
        String actualStockCurrentPrice = "";
        try {
            ...
            test.get().pass("PASS: Navigated to expected stock URL for: " + expected.getStockName());
            ...
            actualStockCurrentPrice = driver.get().findElement(By.cssSelector("span#quoteLtp")).getText();

            Assert.assertEquals(actualStockName, expected.getStockName(), "Stock name mismatch");
            test.get().pass("PASS: expected stockName: " + expected.getStockName() + ", actual stockName: " + actualStockName);
            Assert.assertEquals(actualStockFullName, expected.getStockFullName(), "Stock full name mismatch");
            test.get().pass("PASS: expected stockFullName: " + expected.getStockFullName() + ", actual stockFullName: " + actualStockFullName);

            double tolerance = 2.00;
            double expectedPrice = Double.parseDouble(expected.getCurrentPrice().replaceAll("[^\\d.]", ""));
            double actualPrice = Double.parseDouble(actualStockCurrentPrice.replaceAll("[^\\d.]", ""));
            if (Math.abs(expectedPrice - actualPrice) <= tolerance) {
                test.get().pass("PASS: Expected stockCurrentPrice: " + expected.getCurrentPrice() +
                        ", Actual stockCurrentPrice: " + actualStockCurrentPrice +
                        ", match within tolerance: " + tolerance);
            } else {
                test.get().fail("FAIL: Expected stockCurrentPrice: " + expected.getCurrentPrice() +
                        ", Actual stockCurrentPrice: " + actualStockCurrentPrice +
                        ", does not match within tolerance: " + tolerance);
                hasSoftFailure.set(true);
            }
        } catch (Throwable e) {
            hasSoftFailure.set(true);
            test.get().fail("FAIL: Test search for stock information: " + expected.getStockName() + " failed - " + e.getMessage());
        }
        ...
        try {
            ...
            test.get().pass("PASS: Test 52 Week High and Low prices for stock: " + expected.getStockName());
            extentTest.info("52 Week High price: " + high + ", 52 Week Low price: " + low);
        } catch (Throwable e) {
            hasSoftFailure.set(true);
            test.get().fail("FAIL: Test 52 Week High and Low prices for stock: " + expected.getStockName() + " failed - " + e.getMessage());
        }

        // Calculate profit or loss
        try {
            String priceText = driver.get().findElement(By.cssSelector("span#quoteLtp")).getText().replaceAll("[^\\d.]", "");
            double currentPrice = Double.parseDouble(priceText);

            String buyPriceText = expected.getBuyingPrice().replaceAll("[^\\d.]", "");
            ...
            if (profitOrLoss > 0) {
                extentTest.pass("PASS: Profit of " + profitOrLoss + " for stock: " + expected.getStockName());
            } else if (profitOrLoss < 0) {
                extentTest.warning("WARNING: Loss of " + Math.abs(profitOrLoss) + " for stock: " + expected.getStockName());
            } else {
                extentTest.info("INFO: No profit or loss for stock: " + expected.getStockName());
            }
            Assert.assertFalse(Double.isNaN(profitOrLoss), "Profit/Loss should be a valid number");
            test.get().pass("PASS: Test Calculate profit or loss for stock: " + expected.getStockName());
        } catch (Throwable e) {
            hasSoftFailure.set(true);
            test.get().fail("FAIL: Test Calculate profit or loss for stock: " + expected.getStockName() + " failed - " + e.getMessage());
        }
